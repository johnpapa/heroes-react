...
function HeroDetail(props) {
  const { handleCancelHero } = props;
  const [ hero, setHero ] = useState(Object.assign({}, props.hero));

  useEffect(() => {
    if (!hero) {
      props.history.push('/'); // no hero, bail out of Details
    }
  }, [hero, props])
